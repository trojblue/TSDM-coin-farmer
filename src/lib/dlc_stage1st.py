...
def refresh_cookie_s1(username: str, password:str):
    ...
    if username and password:
        ...
        display_warning("刷新cookie: 未找到S1_CREDENTIALS, 想要自动获取cookie请按readme设置好该变量")
...
def do_read_s1_single(cookie:List):
    ...
    if ("动漫论坛 -  Stage1st") in read_response.text:
        display_info("%s S1: 状态正常"%datetime.now())
    else:
        display_error("%s ====S1: 状态异常===="%datetime.now())


def do_read_s1_all():
    s1_cookie = get_cookies_by_domain(s1_domain)
    display_info("正在s1刷在线时间...")

    for user in s1_cookie.keys():
        display_info("%s 正在s1阅读: %s"%(datetime.now(), user))
        try:
            do_read_s1_single(s1_cookie[user])
        except Exception as e:
            display_error("====S1阅读失败, 继续运行: %s==="%e)
